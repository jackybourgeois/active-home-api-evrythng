 ...
 */
@ComponentType(version = 1, description = "Mock up to test Evrythng API.")
public class TesterEvrythngAPI extends ComponentTester {
    ...
    private String[] fakeMetrics = {"kettle.power", "ev.soc",
            "kettle.temperature", "inverter.pv.DC1"};
    ...
    private void sendRandomData() {
        for (String metric : fakeMetrics) {
            DataPoint dp = new DataPoint(metric,
                    getCurrentTime(), rand.nextDouble() + "");
            Notif notif = new Notif(getFullId(),
                    getNode() + ".context", getCurrentTime(), dp);
            logInfo(notif.toString());
            sendNotif(notif);
        }
    }
